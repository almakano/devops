...
zcat /home/*/www/*/logs/*nginx*.log*.gz | grep -E '(0x%5B%5D=|wp-admin|wp-config|\.env)' | grep -v '" 200 ' | awk '{print $2}' | grep -vE '(\w|127.0.0.1)' | sort | uniq >> /etc/ipset/blacklist_web.txt;
cat /home/*/www/*/logs/*nginx*.log | grep -E '(0x%5B%5D=|wp-admin|wp-config|\.env)' | grep -v '" 200 ' | awk '{print $2}' | grep -vE '(\w|127.0.0.1)' | sort | uniq >> /etc/ipset/blacklist_web.txt;
